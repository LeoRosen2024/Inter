...
async function openScriptForReel(reelId,title){
  ...
  const scriptTitle=viewRoot.querySelector('.script-workspace-head h1');if(scriptTitle)scriptTitle.textContent=activeReelRecord?.title||title;const hookCopy=viewRoot.querySelector('[data-script-block=hook] .saved-copy');if(hookCopy)hookCopy.textContent=activeReelRecord?.transcript||activeReelRecord?.description||'Transcript wird über die Schaltfläche im Reel-Detail geladen.';const scriptCopy=viewRoot.querySelector('[data-script-block=script] .saved-copy');if(scriptCopy)scriptCopy.textContent=activeReelRecord?.transcript||activeReelRecord?.description||'Transcript wird über die Schaltfläche im Reel-Detail geladen.';const hookHeader=viewRoot.querySelector('[data-script-block=hook] header');if(hookHeader&&!hookHeader.querySelector('[data-action=transcript]'))hookHeader.insertAdjacentHTML('beforeend','<button class=transcript-button data-action=transcript>Получить transcript</button>');
...
async function startApifyImport(){
  ...
  try{const job=await api.createApifyImport({source_url:activeReelRecord.source_url,limit:1,actor_input:{username:[activeReelRecord.source_url],resultsLimit:1,includeTranscript:true,includeDownloadedVideo:false}});notify('Transcript wird geladen …');for(let i=0;i<36;i++){await new Promise(r=>setTimeout(r,3000));const current=await api.getImport(job.id);if(current.status==='succeeded'){activeReelRecord=await api.getReel(activeReelRecord.id);reelRecordCache.set(activeReelRecord.id,activeReelRecord);if(location.hash==='#scripts'){viewRoot.innerHTML=scriptWorkspaceTemplate();const scriptTitle=viewRoot.querySelector('.script-workspace-head h1');if(scriptTitle)scriptTitle.textContent=activeReelRecord.title;const hookCopy=viewRoot.querySelector('[data-script-block=hook] .saved-copy');if(hookCopy)hookCopy.textContent=activeReelRecord.transcript||activeReelRecord.description;const hookHeader=viewRoot.querySelector('[data-script-block=hook] header');if(hookHeader)hookHeader.insertAdjacentHTML('beforeend','<button class=transcript-button data-action=transcript>Transcript aktualisieren</button>')}else{viewRoot.innerHTML=reelDetailTemplate(activeReelRecord,activeReelRecord.title,'');mountTranscriptControl()}notify('Transcript wurde geladen');return}if(current.status==='failed'){notify('Transcript konnte nicht geladen werden');return}}}catch{notify('Transcript konnte nicht angefordert werden')}}
